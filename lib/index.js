...
export const beautifyNumber = (number, decimals) => {
  // make it a float with all decimals possible
  const floatFormatted = parseFloat(number)

  // trim decimals if length is big enough
  const DESIRED_LENGTH = 3
  const integralLength = Math.ceil(Math.log10(Math.abs(floatFormatted + 1)))

  const desiredDecimals = decimals || DESIRED_LENGTH - integralLength

  // parseFloat to remove trailing zeros
  const withDecimalsTrimmed = parseFloat(
    floatFormatted.toFixed(desiredDecimals >= 0 ? desiredDecimals : 0)
  )

  // add commas to 1000+ numbers
  const withCommas = addCommasToNumber(withDecimalsTrimmed)
  return withCommas
}
...
export const badInputsCheck = (userAssets, { tokenId, amount }) => {
  const noId = !tokenId
  if (noId) return `Please enter a valid token`

  const noAmount = !amount
  if (noAmount) return `Please enter a valid amount`
